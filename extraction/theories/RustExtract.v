...
Definition fresh_ty_arg_name (name : name) (Γ : list ident) : PrettyPrinter ident :=
  ...
  | nNamed name => ret (capitalize (fresh (get_ty_arg_name name) (Γ ++ used_names)))
  end.
...
Fixpoint print_fundecl (Γ : list ident) (t : box_type) (body : term) : PrettyPrinter unit :=
  let fix print_args ty := 
    match ty with
    | TBox => append type_box_symbol
    | TAny => append any_type_symbol
    | TArr dom cod =>
      (* arg_name <- fresh_ident "a" Γ;;  *)
      (* append (arg_name ++ " : ");; *)
      (print_type Γ dom);;
      append ", ";;
      print_type Γ cod
  ...
  (* match t, body with
  | TArr dom cod, tFix [d] _ => 
    print_parenthesized true
      (print_args dom (dbody d));;
    append " -> " ;;
    print_type Γ cod
  | _,_ =>
    append "() -> ";;
...
instead print
   foo x y z = z
*)
Definition get_funarg_names
         (Γ : list ident)
         (name : ident)
         (t : term)
         (pp : list ident -> PrettyPrinter unit) : PrettyPrinter unit :=
  let fix aux Γ t :=
      match t with
      | tLambda arg_name t =>
  ...
  match t with
  | tFix [d] 0 => pp (aux (name :: Γ) (dbody d))
  | _ => pp []
  end.
  ...
  | tCase (ind, npars) discriminee branches =>
    ...
    (fix print_branches
       ...
       | (arity, t) :: branches, (ctor_name, data) :: ctors =>
         ...
         append (get_ty_name (inductive_mind ind)) ;;
         append "::" ;;
         ...
         (fix print_branch (n : nat) (Γ : list ident) (t : term) (parens : bool) {struct t} :=
            match n with
            | 0 =>
              append (if parens then ")" else "");;
              append " =>";;
              append_nl_and_indent;;
            ...
            | S n =>
              ...
              | tLambda name t =>
                ...
                if parens then
                  append ("" ++ name);;
                  print_branch n (name :: Γ) t false
                else 
                  append ("(" ++ name);;
                  print_branch n (name :: Γ) t true
...
Definition print_constant_body
  ...
  | Some body =>
    ...
    (* TODO: merge type declaration and define term into a single *)
    match type with
    | (type_vars, ty) =>
      append ml_name;;
      ...
      print_parenthesized_with "<" ">" 
        (0 <? List.length Γrev) 
        (monad_fold_left (fun _ id => append (id ++ ",")) Γrev tt) ;;
       
      print_fundecl (rev Γrev) ty body
    end;;

    let name := get_fun_name name in
    push_use name;;
    print_parenthesized_with " {" "}" true
...
Definition print_mutual_inductive_body
  ...
  (fix print_ind_bodies
     ...
     | oib :: l =>
       ...
       print_parenthesized_with "<" ">"
         (0 <? List.length Γ) 
         (monad_fold_left (fun a name => ret a ;; append name) Γ tt) ;;
       append " {" ;;
       ...
       (fix print_ind_ctors (ctors : list (ident * list box_type)) postfix :=
          match ctors with
          | [] => ret tt
          | (name, data) :: ctors =>
            append_nl_and_indent;;
